...
func NewAOIManager(minX, maxX, cntsX, minY, maxY, cntsY int) *AOIManager {
	...
	for y := 0; y < cntsY; y++ {
		for x := 0; x < cntsX; x++ {
			...
			//给aoiManager添加一个格子
			aoiMngr.grids[gid] = NewGrid(gid,
				aoiMngr.MinX+x*aoiMngr.GridWidth(),
				aoiMngr.MinX+(x+1)*aoiMngr.GridWidth(),
				aoiMngr.MinY+y*aoiMngr.GridHeight(),
				aoiMngr.MinY+(y+1)*aoiMngr.GridHeight())
		}
	}
